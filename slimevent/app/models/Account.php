...
class Account{
	...
	static function check_user_pwd($uid, $pwd)
	{
		$r = DB::sql('SELECT * FROM `users` WHERE `id` = :id AND `pwd` = :pwd', array(
			':id' => trim($uid), ':pwd' => self::encrypt_pwd($pwd)
		));
		...
		if( count($r) == 0 )
			...
			Sys::error(F3::get('USERS_NAME_SAME_CODE'), $uid);
	}
	/**
	 * 加密密码
	 * @param $pwd
